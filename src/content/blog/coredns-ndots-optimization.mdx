...
Most application names do not meet that bar. A name such as `mysvc.example.com` looks complete to a human, but with `ndots:5` the resolver first appends the pod's configured search suffixes and tries several expanded names that almost certainly do not exist.
...
That simpler design was feasible because the domains that needed interception were only a handful. If the target set had been broad or dynamic, `autopath` would have been the more appropriate design.
...
- CoreDNS `autopath` plugin documentation: [https://coredns.io/plugins/autopath/](https://coredns.io/plugins/autopath/)
